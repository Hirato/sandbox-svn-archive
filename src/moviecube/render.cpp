...
//	    rendermodel(NULL, name   , anim, d->feetpos(), yaw, pitch, 0, flags, d, attached[0].tag ? attached : NULL, basetime, 0, fade);
	    rendermodel(mdlname, anim, d->feetpos(), yaw, pitch, 0, flags, d, attachments, basetime, 0, fade); // Fix by Protagoras 20101119
	}


    void renderplayer(DynamicEntity *d, const PlayerModelInfo &mdl, float fade)
    {
    ...
    void rendergame()
    {
        ...
        loopv(players)
        {
            ...
            renderplayer(d, getplayermodelinfo(d), 1);
        ...
        loopv(ragdolls)
        {
            ...
            renderplayer(d, getplayermodelinfo(d), fade);
        }
        if(isthirdperson() && !followingplayer()) renderplayer(player1, getplayermodelinfo(player1), 1);
        rendermovables();
