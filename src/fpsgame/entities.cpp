...
//                d->vel = v;
    ...
    static inline void setuptriggerflags(fpsentity &e)
    {
        ...
        if(checktriggertype(e.attr[5], TRIG_COLLIDE|TRIG_DISAPPEAR)) e.flags |= extentity::F_NOSHADOW;
        if(!checktriggertype(e.attr[5], TRIG_COLLIDE)) e.flags |= extentity::F_NOCOLLIDE;
        switch(e.triggerstate)
        {
            case TRIGGERING:
                if(checktriggertype(e.attr[5], TRIG_COLLIDE) && lastmillis-e.lasttrigger >= 500) e.flags |= extentity::F_NOCOLLIDE;
                break;
            case TRIGGERED:
                if(checktriggertype(e.attr[5], TRIG_COLLIDE)) e.flags |= extentity::F_NOCOLLIDE;
                break;
    ...
    void resettriggers()
    {
        loopv(ents)
        {
            ...
            if(e.type != ET_MAPMODEL || !validtrigger(e.attr[5])) continue;
    ...
    void unlocktriggers(int tag, int oldstate = TRIGGER_RESET, int newstate = TRIGGERING)
    {
        loopv(ents)
        {
            fpsentity &e = *(fpsentity *)ents[i];
            if(e.type != ET_MAPMODEL || !validtrigger(e.attr[5])) continue;
            if(e.attr[6] == tag && e.triggerstate == oldstate && checktriggertype(e.attr[5], TRIG_LOCKED))
            {
                if(newstate == TRIGGER_RESETTING && checktriggertype(e.attr[5], TRIG_COLLIDE) && overlapsdynent(e.o, e.attr[7] ? e.attr[7] : 20)) continue;
                e.triggerstate = newstate;
                e.lasttrigger = lastmillis;
                if(checktriggertype(e.attr[5], TRIG_RUMBLE)) playsound(S_RUMBLE, &e.o);
            }
        }
    }
            ...
            switch(e.triggerstate)
            {
                ...
                case TRIGGER_RESETTING:
                    if(lastmillis-e.lasttrigger>=1000)
                    {
                        if(e.attr[6])
                        {
                            if(e.triggerstate == TRIGGERING) unlocktriggers(e.attr[6]);
                            else unlocktriggers(e.attr[6], TRIGGERED, TRIGGER_RESETTING);
                        }
                        if(checktriggertype(e.attr[5], TRIG_DISAPPEAR)) e.triggerstate = TRIGGER_DISAPPEARED;
                        else if(e.triggerstate==TRIGGERING && checktriggertype(e.attr[5], TRIG_TOGGLE)) e.triggerstate = TRIGGERED;
                ...
                case TRIGGER_RESET:
                    if(e.lasttrigger)
                    {
                        if(checktriggertype(e.attr[5], TRIG_AUTO_RESET|TRIG_MANY|TRIG_LOCKED) && e.o.dist(o)-player1->radius>= (e.attr[7] ? e.attr[7] : (checktriggertype(e.attr[5], TRIG_COLLIDE)  ? 20 : 12)))
                            e.lasttrigger = 0;
                        break;
                    }
                    else if(e.o.dist(o)-player1->radius>= (e.attr[7] ? e.attr[7] :(checktriggertype(e.attr[5], TRIG_COLLIDE) ? 20 : 12))) break;
                    else if(checktriggertype(e.attr[5], TRIG_LOCKED))
                    {
                        if(!e.attr[6]) break;
                        doleveltrigger(e.attr[6], -1);
                        e.lasttrigger = lastmillis;
                        break;
                    }
                    ...
                    setuptriggerflags(e);
                    if(checktriggertype(e.attr[5], TRIG_RUMBLE)) playsound(S_RUMBLE, &e.o);
                    if(checktriggertype(e.attr[5], TRIG_ENDSP)) endsp(false);
                    if(e.attr[6]) doleveltrigger(e.attr[6], 1);
                    break;
                case TRIGGERED:
                    if(e.o.dist(o)-player1->radius< (e.attr[7] ? e.attr[7] : (checktriggertype(e.attr[5], TRIG_COLLIDE) ? 20 : 12)))
                    {
                        if(e.lasttrigger) break;
                    }
                    else if(checktriggertype(e.attr[5], TRIG_AUTO_RESET))
                    {
                        if(lastmillis-e.lasttrigger<6000) break;
                    }
                    else if(checktriggertype(e.attr[5], TRIG_MANY))
                    {
                        e.lasttrigger = 0;
                        break;
                    }
                    else break;
                    if(checktriggertype(e.attr[5], TRIG_COLLIDE) && overlapsdynent(e.o, e.attr[7] ? e.attr[7] : 20)) break;
                    e.triggerstate = TRIGGER_RESETTING;
                    e.lasttrigger = lastmillis;
                    setuptriggerflags(e);
                    if(checktriggertype(e.attr[5], TRIG_RUMBLE)) playsound(S_RUMBLE, &e.o);
                    if(checktriggertype(e.attr[5], TRIG_ENDSP)) endsp(false);
                    if(e.attr[6]) doleveltrigger(e.attr[6], 0);
                    break;
            }
        }
    }
    ...
    void entradius(extentity &e, bool &color)
    {
        switch(e.type)
        {
            ...
            case MAPMODEL:
                if(validtrigger(e.attr[5])) renderentring(e, e.attr[7] ? e.attr[7] : checktriggertype(e.attr[5], TRIG_COLLIDE) ? 20 : 12);
                break;
        }
    }
    ...
    {
        extentity &e = *ents[i];
        if(e.type == ET_MAPMODEL && validtrigger(e.attr[5]))
        {
