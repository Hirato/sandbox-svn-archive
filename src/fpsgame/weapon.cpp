...
namespace game
{
        ...
        loopv(bouncers)
        {
            ...
            if(stopped)
            {
                if(bnc.bouncetype==BNC_GRENADE)
                {
                    ...
                    adddecal(DECAL_WATERBURN, bnc.o, vec(0, 0, 1), guns[GUN_GL].exprad/2);
                    if(bnc.local)
    ...
    void explode(bool local, fpsent *owner, const vec &v, dynent *safe, int damage, int gun)
    {
        ...
        particle_fireball(v, guns[gun].exprad, gun!=GUN_GL ? PART_EXPLOSION : PART_EXPLOSION_BLUE, gun!=GUN_GL ? -1 : int((guns[gun].exprad-4.0f)*15), gun!=GUN_GL ? 0x00B2EE : 0x00B2EE, 4.0f);
    ...
    void projsplash(projectile &p, vec &v, dynent *safe, int damage)
    {
        ...
        else
        {
            ...
            adddecal(DECAL_WATERBURN, v, vec(p.dir).neg(), guns[p.gun].exprad/2);
        }
    }
            ...
            case GUN_RL:
                loopv(projs)
                {
                    ...
                    if(p.gun == gun && p.owner == d && p.id == id && !p.local)
                    {
                        ...
                        adddecal(DECAL_WATERBURN, pos, vec(p.dir).neg(), guns[gun].exprad/2);
                        projs.remove(i);
                        break;
                    }
                }
            ...
            case GUN_GL:
                loopv(bouncers)
                {
                    ...
                    if(b.bouncetype == BNC_GRENADE && b.owner == d && b.id == id && !b.local)
                    {
                        ...
                        adddecal(DECAL_WATERBURN, pos, vec(0, 0, 1), guns[gun].exprad/2);
                        delete bouncers.remove(i);
                        break;
                    }
                }
    ...
    void shoteffects(int gun, const vec &from, const vec &to, fpsent *d, bool local, int id, int prevaction)     // create visual effect from a shot
    {
        ...
        switch(gun)
        {
            ...
            case GUN_SG:
            {
                ...
                loopi(guns[gun].rays)
                {
                    particle_splash(PART_SPARK, 20, 250, rays[i], 0x00B2EE, 0.24f);
                    particle_flare(hudgunorigin(gun, from, rays[i], d), rays[i], 300, PART_STREAK, 0x00B2EE, 0.28f);
                    if(!local) adddecal(DECAL_RIPPLE, rays[i], vec(from).sub(rays[i]).normalize(), 2.0f);
                }
    ...
    void raydamage(vec &from, vec &to, fpsent *d)
    {
        ...
        if(guns[d->gunselect].rays > 1)
        {
            ...
            loopi(maxrays)
            {
                ...
                else adddecal(DECAL_RIPPLE, rays[i], vec(from).sub(rays[i]).normalize(), 2.0f);
            }
