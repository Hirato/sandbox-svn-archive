...
namespace server
{
    ...
    struct clientinfo
    {
        ...
        clientinfo() : getdemo(NULL), getmap(NULL), getcfg(NULL), clipboard(NULL), authchallenge(NULL), authkickreason(NULL) { reset(); }
    ...
    {
        execfile("fpsserver-init.cfg", false);
    }
    ...
    bool setmaster(clientinfo *ci, bool val, const char *pass = "", const char *authname = NULL, const char *authdesc = NULL, int authpriv = PRIV_MASTER, bool force = false, bool trial = false)
    {
        ...
        if(val && authname)
        {
            ...
            else formatstring(msg)("%s claimed %s as '\fs\f5%s\fr'", colorname(ci), name, authname);print(msg);
        }
        else
        {
    ...
    void parsepacket(int sender, int chan, packetbuf &p)     // has to parse exactly each byte of the packet
    {
        ...
        case N_TEXT:
        {
            ...
            case N_EDITENT:
            {
                ...
                int type = getint(p);
                int num = getint(p);
                loopk(num) { if(p.overread()) break; getint(p); }
            ...
            case N_KICK:
            {
                ...
                getstring(text, p);
                filtertext(text, text);
                if(trykick(ci, victim, text))
                {
                    print("%s was kicked by %s (reason: %s)", colorname((clientinfo *)getclientinfo(victim)), colorname(ci), text);
                }
            }
