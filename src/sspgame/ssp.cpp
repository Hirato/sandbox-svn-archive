...
namespace game
{
	...
	void gameplayhud(int w, int h)
	{
		...
		loopi(numw)
		{
			loopj(numw)
			{
				...
				if(img->deltime - lastmillis <= 500)
					gle::colorf(1, 1, 1, (img->deltime - lastmillis) / 500.0);
				else if(lastmillis - img->starttime <= 200)
					gle::colorf(1, 1, 1, (lastmillis - img->starttime) / 200.0);
				else
					gle::colorf(1, 1, 1, 1);
		...
		if(lastmillis < player1->lastpain + 2000)
			gle::colorf(1, .5, .5, 1); //set colours, he obviously had a recent owie
		else
			gle::colorf(1, 1, 1, 1);
	...
	const char *defaultcrosshair(int index)
	{
		...
		switch(index)
		{
			...
			default: return "packages/crosshairs/empty";
		}
	}
