...
{
	rendermodel(mapmodelname(model), ANIM_MAPMODEL|ANIM_LOOP,
...
void sspmonster::takedamage(int amount, bool immunity)
{
	...
	{
		static string scr;
		formatstring(scr)("enemy_dead_%i", scridx);
		if(identexists(scr)) execute(scr);
		state = CS_DEAD;
	}
...
void sspitem::render()
{
	...
	rendermodel(game::pickups[index]->mdl,
		ANIM_MAPMODEL|ANIM_LOOP, vec(o).add(vec(0, 0, (float)(1 + sin(lastmillis/300.0)))),
		lastmillis/10.0f, 0, 0, MDL_CULL_VFC|MDL_CULL_DIST|MDL_CULL_OCCLUDED
	);
}
...
void sspbox::render()
{
	...
	rendermodel(mapmodelname(mdl), ANIM_MAPMODEL|ANIM_LOOP, pos, yaw, 0, 0, MDL_CULL_VFC|MDL_CULL_DIST|MDL_CULL_OCCLUDED
	);
}
